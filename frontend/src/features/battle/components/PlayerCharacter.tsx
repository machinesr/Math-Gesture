...
// Unified player sprite for both sides of the battlefield. The `side` prop
// drives the attack-lunge direction and horizontal flip; everything else is
// shared. Replaces the previous playermodelleft / playermodelright pair.
export default function PlayerCharacter({
  ...
  const lungeClass = isAttacking
    ? side === "left"
      ? "translate-x-[clamp(2rem,4vw,5rem)] scale-110"
      : "-translate-x-[clamp(2rem,4vw,5rem)] scale-110"
    : "translate-x-0"
  ...
  return (
    <div className={`flex flex-col items-center transition-transform duration-200 ${lungeClass}`}>
      ...
      <img
        src={sprite}
        alt={name}
        className={`w-[clamp(48px,14vw,200px)] md:w-[clamp(80px,10.4vw,200px)] select-none pointer-events-none ${flipClass} ${isAttacking ? "brightness-125" : ""}`}
      />
    </div>
  )
}
